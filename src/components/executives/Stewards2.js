...
export const Stewards2 = ({ navigation, route }) => {
    ...
    const { data, loading, refetch } = useQuery(GET_STANDING_COMMITTEE, {
        variables: {
            unionID: userData?.unionID,
            category: "standingCommittee_second"
        },
        onCompleted: () => {
            if (data.standingCommittee) {
    ...
    return (
        <>
            <SafeAreaView style={{
                ...
                <HeaderInPages title="Second Stewards" />
                {loading ?
