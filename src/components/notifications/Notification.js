...
export const Notification = ({ notification, sendDeletedItem }) => {
    ...
    const swipeRowRef = useRef(null); // Create a ref for SwipeRow
    ...
    const [markNotificationAsRead] = useMutation(MARK_NOTIFICATION, {
    ...
    const [delNotification, { loading }] = useMutation(DELETE_NOTIFICATION, {
        ...
        onCompleted: () => {
            setOnDel(true);
            if (swipeRowRef.current) {
                swipeRowRef.current.closeRow(); // Close the swipe on success
            }
        },
        onError: () => {
    ...
    return (
        <>
            {onDel ? null :
                <View style={styles.containerForSwipe}>
                    <SwipeRow
                        ref={swipeRowRef} // Attach ref to SwipeRow
                        closeOnRowPress={true}
                        directionalDistanceChangeThreshold={5} // Adjusted for more sensitive swipe
                        rightOpenValue={-screenWidth / 1.5} // Slightly wider for better swipe
                        disableRightSwipe={false} // Enable if swipe right needed
                        onRowDidOpen={onRowSwipe} // Delete when the row is swiped
                    >
                        {/* Hidden item */}
