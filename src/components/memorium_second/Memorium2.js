...
import { PIN_MEMORIUM, LIKE_MEMORIUM, SHOW_PIN_MEMORIUM } from "../../../graph/mutations/memorium";
import HTMLView from "react-native-htmlview";
import LottieView from "lottie-react-native";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { HeaderInPages } from "../header/HeaderInPages";
import { GET_MEMORIUM, SINGLE_MEMORIUM } from "../../../graph/queries/memorium";
import { useNavigation } from '@react-navigation/native';

export const Memorium2 = () => {
    ...
    const [memoriumFeed, setMemoriumFeed] = useState([]);


    const { data, loading, error, refetch } = useQuery(GET_MEMORIUM, {
        variables: {
            unionID: unionData.id,
            page: 1,
            perpage: 999,
            category: "memorium_second",
        },
        onCompleted: () => {
            if (
                data &&
                data.memoriumFeed &&
                data.memoriumFeed.data &&
                data.memoriumFeed.data.length
            ) {
                setMemoriumFeed([...data.memoriumFeed.data]);
                setRefreshing(false);
            }
            if (
                data &&
                ...
                data.memoriumFeed.data &&
                data.memoriumFeed.data.length &&
                data.memoriumFeed.pinned &&
                data.memoriumFeed.pinned.length
            ) {
                setMemoriumFeed([...data.memoriumFeed.pinned, ...data.memoriumFeed.data]);
                setRefreshing(false);
            } else {
                if (
                    data.memoriumFeed.pinned &&
                    ...
                    console.log('only pinned');
                    setMemoriumFeed([...data.memoriumFeed.pinned]);
                    setRefreshing(false);
                }
            }
    ...
    const openComments = (newsID, userData, commentCount, isLiked) => {
        // console.log(newsFeed.find(item => item.id === newsID));
        navigation.navigate("CommentsOfMemorium2", {
            news: memoriumFeed.find((item) => item.id === newsID),
            userData: userData,
            logoURL: logoURL,
    ...
    return (
        <SafeAreaView style={{
            ...
            <HeaderInPages title="Second Memoriam" />
            {loading && memoriumFeed.length === 0 ? (
                <View
...
const MemoriumFeed = ({ navigation, setNewsArrToNull, news, getNews, showErr, openComments, asUnion }) => {
    ...
    const [likeNewsItem] = useMutation(LIKE_MEMORIUM, {
        variables: {
    ...
    const [pinNews] = useMutation(PIN_MEMORIUM, {
        variables: {
            unionID: userData?.unionID,
            memoriumID: news?.id
        },
    ...
    const [showPin] = useMutation(SHOW_PIN_MEMORIUM, {
        onError: (err) => {
            console.log(err);
        },
        onCompleted: () => {
    ...
    const { data, loading, error, refetch } = useQuery(SINGLE_MEMORIUM, {
        ...
        onCompleted: (data) => {
            setCommentCount(data.singleMemorium.comments ? data.singleMemorium.comments.length : '');
